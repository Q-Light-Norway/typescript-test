...
export class Connection {
  ...
  constructor(socket: Websocket, req?, logger?: Logger) {
    this.socket = socket;
    this.req = req;
    this.requests = {};
    this.logger = logger;
    ...
    if (req) {
      ...
      const ip =
        req && ((req.connection && req.connection.remoteAddress) || req.headers["x-forwarded-for"]);
  ...
  async onMessage(message) {
    ...
    try {
      [messageType, messageId, commandNameOrPayload, commandPayload, errorDetails] =
        JSON.parse(message);
    } catch (err: any) {
    ...
    if (process.env.NODE_ENV === "development") {
      console.log("[OCPP] Received Message:  ", message);
    }
  ...
  sendMessage(messageId, command, messageType = CALLRESULT_MESSAGE) {
    ...
    if (process.env.NODE_ENV === "development") {
      console.log("[OCPP] Sending Message:  ", messageId, command);
    }
